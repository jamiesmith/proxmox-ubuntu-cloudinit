...
# curl -s "https://raw.githubusercontent.com/jamiesmith/proxmox-ubuntu-cloudinit/create-ubuntu-cloud-template.sh?$(date +%s)" | /bin/bash -s
...
if [[ ! -f ./jammy-server-cloudimg-amd64.img ]]
then
...
if [[ -f ./templates/cloudinit/template_cloudinit.yml ]]
then
    ...
    # JRS
    cp /tmp/template_cloudinit.yml ./templates/cloudinit/template_cloudinit.yml
    envsubst < /tmp/template_cloudinit.yml > $VM_SNIPPET_PATH/template-user-data.yml
    rm -f /tmp/template_cloudinit.yml
fi

if [[ -f ./templates/cloudinit/clone_cloudinit.yml ]]
then
    ...
    # JRS
    cp /tmp/clone_cloudinit.yml ./templates/cloudinit/clone_cloudinit.yml
    rm -f /tmp/clone_cloudinit.yml
fi

echo "creating new VM..."
qm create $VM_ID --memory 2048 --cores 4 --machine q35 --bios ovmf --net0 virtio,bridge=vmbr0,tag=100
